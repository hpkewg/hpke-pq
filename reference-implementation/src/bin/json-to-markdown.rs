...
fn convert_test_vector_to_markdown(tv: &TestVector) -> String {
    ...
    let setup_section = if tv.mode == 1 { "PSK Setup Information" } else { "Base Setup Information" };
    output.push_str(&format!("### {}\n", setup_section));
    output.push_str("~~~\n");
